...
- 더 효율적으로 코드 쓰고 싶다면 조건은 where절로 따로 빼지말고 on절에 쓰기!<br/>
...
여기서 좀 더 효율적으로 코드를 짜고 싶다면, 조건을 WHERE절로 따로 빼지 말고 <br/>
ON에 조건을 같이 넣을 것! (ON B.AUTHOR_ID = A.AUTHOR_ID AND CATEGORY = '경제') <br/>
이렇게 되면 join할 때 조건에 해당되는 애들만 join이 된다!